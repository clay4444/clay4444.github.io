...
加入一个数的时候，扔一次骰子，扔出1在第一层，扔出0、1在第二层，扔出0、0、1在第三层，依次类推。左边代表系统最小，所有数字最大的层数就是最左边的层数；

如果一个数在L层上（扔骰子扔出来的），如果L层没到最大层，扔骰子扔出来的一个比较小的值， 就从最高层开始找，如果最高层的右边下一个值比当前值要大，往下走，如果下一个值比当前值要小，往右走，走到不能再走了，往下走，在这个过程中如果已经到了L层了，在往下走之前把属于内个值的内个层的点建上，然后往下走，依次建立出来属于新的值的所有层的点；

<br/>

假设当前的情况如图示

{% asset_img picture133.jpg %}

<br/>

此时又来了一个8，扔出来了3层，则8加入后的图示为

<br/>

{% asset_img picture144.jpg %}

<br/>

假设某个数进来之后扔出来了最高的层怎么办？如下图所以，一开始只有7，且层数只有3层，8进来之后扔出了5层，此时的情况如图，

{% asset_img picture13.jpg %}

<br/>

首先左边代表系统最小的先建出5层，4、5层指向null，null代表系统最大，所以4、5层指向null的中间建出8的节点，然后向下走，在7的后面建立8的前三层，最终的结果如图所示
...
从数据分布来看，第一层（最底层）的数最多，层数往上数目会越来越少，这是概率决定的，不会每个数扔出来的层数都很高，高层的点的数目会特别少（概率决定的），而在查询一个数或者建立一个数的层数的时候，是从（高层）开始找的，在高层一跨，就会跨过底层非常多的数，而且这个跨的过程是不会回退的，只会往右或者往下，这样就省掉了从底层开始一个一个开始遍历的开销，提高了效率

这个结构在分布上，就是一个和输入规律没关系的事情，因为一个key，经过一个随机函数，二分之一的比率扔出0和1，只有一层的数会很多， 高层的数会越来越少，在高层跨一个的时候，会省掉从第一层开始遍历很多的工作量，而且跨的过程中，是不会回退的，只会往下或者往右走。也就是用二分之一这个概率来优化的效率；
